...
- **メニューバーはNSStatusItem自前管理（MenuBarController.swift）。SwiftUIのMenuBarExtraに戻さないこと**
  - MenuBarExtraはアイコンをドラッグで取り外した状態がmacOSに記憶されると、以後の起動で即終了する不具合がある（2026-07-15に遭遇）
- **バンドルIDは com.johosauce.key-renda-kidou。旧ID（com.morifumi.〜）に戻さないこと**
  - 旧IDにはmacOSが「アイコン取り外し＝非表示」を内部記録しており、そのIDだとメニューバーアイコンが二度と表示されない（2026-07-16に遭遇。設定UIやdefaultsからは消せず、ID変更でのみ解決）
- 入力監視の許可はビルドごとに再付与が必要（ad-hoc署名のため）。build-app.sh --install が古い許可エントリを自動リセットする
- 設定ウィンドウは`hosting.sizingOptions = []`でサイズ固定（SwiftUIの自動リサイズが描画サイクルと衝突してクラッシュするため）
